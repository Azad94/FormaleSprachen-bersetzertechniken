...
import ast.*;
import lexer_parser.RecDesLexer;
...
public class IndepInorderPrintVisitor implements IPrintVisitor{
	...
	private String choosePrintByToken(ExprNode node){
		...
		switch( node.getToken().type ){
			case(RecDesLexer.PLUS):
			case(RecDesLexer.MINUS):
			case(RecDesLexer.DIV):
			case(RecDesLexer.MULTI):
				str = print((BinaryNode)node);
				break;
			case(RecDesLexer.INTEGER):
				str = print((AtomicNode)node);
				break;
			case(RecDesLexer.UMINUS):
